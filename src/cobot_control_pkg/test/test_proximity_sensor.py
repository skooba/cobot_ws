...
class TestProximitySensor(unittest.TestCase):
    ...
    def test_node_initialization(self):
        ...
        self.assertEqual(self.node.get_parameter("min_distance_mm").value, 100.0)
        self.assertEqual(self.node.get_parameter("max_distance_mm").value, 1000.0)
        self.assertEqual(self.node.get_parameter("publish_rate_hz").value, 0.25)
    ...
    def test_setting_custom_parameters(self):
        ...
        self.assertEqual(self.node.get_parameter("min_distance_mm").value, 50.0)
        self.assertEqual(self.node.get_parameter("max_distance_mm").value, 1200.0)
    ...
    def test_publish_proximity_data(self):
        ...
        self.node.set_parameters([
            Parameter('publish_rate_hz', Parameter.Type.DOUBLE, nominal_frequency)
        ])
        ...
        while len(self.received_messages) < min_expected_messages and (time.time() -start_time) < 5:
            rclpy.spin_once(self.node, timeout_sec=0.1)
        
        # Verify we received enough messages
        num_messages = len(self.received_messages)
        self.assertGreaterEqual(num_messages, min_expected_messages, f"Expected a minimum of {min_expected_messages} but received {num_messages}")

        # Calculate the frequency that the messages were being received at
        approximate_freq = (num_messages - 1) /(message_timestamps[-1] - message_timestamps[0])
    ...
    def test_publish_proximity_data_method(self):
        ...
        # Create subscriber
        test_subscriber = self.node.create_subscription(
    ...
    def test_parameter_callback_method(self):
        ...
        test_cases = [
            ...
            {'param': 'min_distance_mm', 'value': 50.0, 'should_succeed': True},
            {'param': 'max_distance_mm', 'value': 2000.0, 'should_succeed': True},
            {'param': 'publish_rate_hz', 'value': 1.0, 'should_succeed': True},
            #Invalid parameter changes
            {'param': 'min_distance_mm', 'value': -1.0, 'should_succeed': False}, # Invalid negative value  
            {'param': 'max_distance_mm', 'value': 1.0, 'should_succeed': False},  # Invalid: less than than min_distance_mm
            {'param': 'publish_rate_hz', 'value': 0.0, 'should_succeed': False}   # Invalid zero value
        ] 
        ...
        for case in test_cases:
            with self.subTest(key=case):
                ...
                result = self.node.proximity_sensor_parameters_callback([param])
                ...
                if case['should_succeed']:
                    ...
                    if case['param'] == 'min_distance_mm':
                        self.assertEqual(self.node.min_distance_mm, case['value'])
                    elif case['param'] == 'max_distance_mm':
                        self.assertEqual(self.node.max_distance_mm, case['value'])
                    elif case['param'] == 'publish_rate_hz':
                        self.assertEqual(self.node.publish_rate_hz, case['value'])
                else:
                    # Verify the instance variable was NOT updated for invalid parameters
                    if case['param'] == 'min_distance_mm':
                        self.assertEqual(self.node.min_distance_mm, original_min_distance_mm)
                    elif case['param'] == 'max_distance_mm':
                        self.assertEqual(self.node.max_distance_mm, original_max_distance_mm)
                    elif case['param'] == 'publish_rate_hz':
                        self.assertEqual(self.node.publish_rate_hz, original_publish_rate_hz)
