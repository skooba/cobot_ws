...
# ROS2 imports and internal imports
...
class TestEmergencyStop(unittest.TestCase):
    ...
    def test_node_initialization(self):
        ...
        self.assertFalse(
            self.node.e_stop_active,
            'Initial state of the estop should be active',
        )
    ...
    def test_emergency_stop_activation(self):
        ...
        self.assertTrue(
            self.node.e_stop_active, 'Emergency stop should be active'
        )
        self.assertGreater(
            len(self.received_estop_messages), 0, 'No estop message received'
        )
        self.assertTrue(
            self.received_estop_messages[-1],
            'Published estop message should be True',
        )
    ...
    def test_emergency_stop_reset(self):
        ...
        # Verify emergency stop is activated
        self.assertFalse(
            self.node.e_stop_active, 'Emergency stop should be reset'
        )
        self.assertGreater(
            len(self.received_estop_messages), 0, 'No estop message received'
        )
        self.assertFalse(
            self.received_estop_messages[-1],
            'Published estop message should be False',
        )
    ...
    def test_toggle_estop_method(self):
        ...
        self.assertTrue(
            self.node.e_stop_active, 'toggle_estop(True) should activate estop'
        )
        ...
        self.assertFalse(
            self.node.e_stop_active,
            'toggle_estop(False) should deactivate estop',
        )
    ...
    def test_publish_estop_data_method(self):
        ...
        self.assertFalse(
            self.received_estop_messages[-1],
            'Should publish False when estop inactive',
        )
        ...
        self.assertTrue(
            self.received_estop_messages[-1],
            'Should publish True when estop active',
        )
    ...
    def test_emergency_stop_workflow_sequence(self):
        ...
        for case in test_cases:
            with self.subTest(
                key=case['key'], expected=case['expected_state']
            ):
                # Send key
                char_msg = Char()
                char_msg.data = ord(case['key'])
                ...
                self.assertEqual(
                    self.node.e_stop_active,
                    case['expected_state'],
                    f'State should be {case["expected_state"]} after'
                    f' \'{case["key"]}\'',
                )
    ...
    def test_multiple_activations_and_resets(self):
        ...
        for i in range(3):
            ...
            self.assertTrue(
                self.node.e_stop_active,
                f'Should remain active after activation {i+1}',
            )
        ...
        for i in range(3):
            ...
            self.assertFalse(
                self.node.e_stop_active,
                f'Should remain inactive after reset {i+1}',
            )
