...
class TestSpeedControl(unittest.TestCase):
    ...
    def test_proximity_callback_functionality(self):
        ...
        for distance in test_distances:
            with self.subTest(distance=distance):
                ...
                # Callback should return False (invalid condition for monitor state)
    ...
    def test_speed_state_publishing(self):
        ...
        speed_subscriber = self.node.create_subscription(
        ...
        estop_publisher = self.node.create_publisher(
        ...
        for scenario in test_scenarios:
            with self.subTest(
                ...
                while len(self.received_speed_states) < 1 and (time.time() - start_time) < 5.0:
                    rclpy.spin_once(self.node, timeout_sec=0.1)

                # Verify we received a speed state
                self.assertGreater(len(self.received_speed_states), 0, 
                                 f"No speed state received for {scenario['description']}")
                
                # Verify the correct state was published
                self.assertEqual(self.received_speed_states[-1], scenario['expected_state'],
                               f"Expected {scenario['expected_state']} for {scenario['description']}")
    ...
    def test_fsm_initial_state(self):
        ...
        self.assertFalse(self.node.fsm.userdata[SpeedStateDataKeys.ESTOP.value])
        self.assertEqual(self.node.fsm.userdata[SpeedStateDataKeys.DISTANCE.value], 0)
