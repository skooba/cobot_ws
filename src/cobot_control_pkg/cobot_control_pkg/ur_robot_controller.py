...
class URRobotController(Node):
    def __init__(self):
        ...
        # ramp_rate: float - the rate at which velocity changes per second (rad/s per second)
        # frequency: float - the frequency at which velocity updates are sent (Hz)
    ...
    def speed_state_callback(self, msg: String) -> None:
        ...
        if self.current_speed_state == SpeedStateOutcomes.STOPPED.value:
            ...
            if self.estop_active:
                self.estop_active = False
            # Stopped state will be sent if object is close to the robot or after the estop is triggered and hasn't been reset
            ...
            if self.estop_active:
                self.estop_active = False
            self.send_ramped_velocity_command([round(VelocityValues.SLOW_SPEED.value, 3)] * 6)
            ...
            if self.estop_active:
                self.estop_active = False
            self.send_ramped_velocity_command([round(VelocityValues.FULL_SPEED.value, 3)] * 6)
    ...
    def send_ramped_velocity_command(
        ...
        if len(target_velocities) != 6:
            self.get_logger().error(f"Need exactly 6 velocity values, got {len(target_velocities)}")
            return
    ...
    def _ramp_worker(
        self, target_velocities: list[float], ramp_rate: float, frequency: float
        ) -> None:
        """
        ...
        max_change_per_step = ramp_rate * dt  # Max velocity change per time step

        # Calculate number of updates needed
        number_updates = ceil(max([abs(a - b) for a, b in zip(target_velocities, self.current_velocities)]) / max_change_per_step)
        

        # Calculate the times at which the updates will be sent
        start_time = time.time()        
        update_times = [start_time + i / frequency for i in range(number_updates)]
        ...
        while not self._stop_ramp_event.is_set() and update_times:
            ...
            if current_time >= update_times[0]:
                # Calculate the next velocities by incrementing towards the target velocities
                for i in range(6):
                    diff = round(target_velocities[i] - self.current_velocities[i], 3)
                    if diff > max_change_per_step:
                        next_velocities[i] = self.current_velocities[i] + max_change_per_step
                    elif diff < -max_change_per_step:
                        next_velocities[i] = self.current_velocities[i] - max_change_per_step
                    else:
                        # on this step, the next velocity is the target velocity
                        next_velocities[i] = target_velocities[i]

                # lock the thread to prevent race condition where estop is triggered between
                # the check for estop and the publishing of the velocity command
                with self._thread_lock:
                    if self.estop_active:
                        return
    ...
    def send_immediate_velocity_command(self, velocities: list[float]) -> None:
        ...
        """
        if len(velocities) != 6:
            self.get_logger().error(f"Need exactly 6 velocity values, got {len(velocities)}")
            return
