...
class ProximitySensorNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('publish_rate_hz', .25)
        ...
        # Create the publisher for which the proximity distance will be published on 
        # Topic: /proximity_distance
        # Message Type: example_interfaces/Float32
        self.publisher_ = self.create_publisher(Float32, "proximity_distance", 10)
        self.get_logger().info("Proximity sensor node initialized. Publishing on /proximity_distance.")

        # Create a timer to periodically publish sensor readings
        self.create_timer(1.0 / self.publish_rate_hz, self.publish_proximity_data)

    def publish_proximity_data(self):
        '''Simulate a random proximity reading and publish the reading to the 'proximity_distance' topic'''
        distance_mm = random.uniform(self.min_distance_mm, self.max_distance_mm)
        msg = Float32()
