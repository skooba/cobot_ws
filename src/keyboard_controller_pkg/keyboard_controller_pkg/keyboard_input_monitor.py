...
class KeyboardInputMonitorNode(Node):
    def __init__(self):
        ...
        self.input_thread = threading.Thread(target=self.monitor_keyboard_input_loop, daemon=True)
        ...
        self.get_logger().info("Keyboard input monitor node initialized. Publishing on /keyboard_input_monitor.")
    ...
    def monitor_keyboard_input_loop(self) -> None:
        ...
        while rclpy.ok():
            ...
            if key == '\x03': # Ctrl+C character
                self.get_logger().info("Ctrl+C detected from keyboard thread. Shutting down get key loop")
