...
# Internal imports
from keyboard_controller_pkg.keyboard_input_monitor import KeyboardInputMonitorNode
...
class TestKeyboardInputMonitor(unittest.TestCase):
    ...
    def test_parameter_declaration(self):
        ...
        with patch.object(KeyboardInputMonitorNode, 'saveTerminalSettings', return_value=None), \
             patch('threading.Thread'), \
             patch.object(KeyboardInputMonitorNode, 'create_publisher', return_value=Mock()):
    ...
    def test_parameter_callback_validation(self):
        ...
        with patch.object(KeyboardInputMonitorNode, 'saveTerminalSettings', return_value=None), \
            ...
            param_invalid = Parameter('key_timeout', Parameter.Type.DOUBLE, -1.0)
    ...
    def test_node_name_and_publisher(self):
        ...
        with patch.object(KeyboardInputMonitorNode, 'saveTerminalSettings', return_value=None), \
            ...
            mock_publisher = Mock()
            with patch.object(KeyboardInputMonitorNode, 'create_publisher', return_value=mock_publisher) as mock_create_publisher:
                node = KeyboardInputMonitorNode()
                
                # Test node name
                self.assertEqual(node.get_name(), "keyboard_input_monitor")
                
                # Test that our specific publisher was created (check for the Char publisher call)
                expected_call = call(Char, "keyboard_input_monitor", 10)
                self.assertIn(expected_call, mock_create_publisher.call_args_list)
